...
<?php
	...
	$sql= "SELECT * FROM lands_houses_apartments WHERE m_ID= '$mID'";
	...
	if($result=$conn->query($sql))
	{
		if($result->num_rows > 0)
		{
			while($row = $result->fetch_assoc())
			{
				...
				echo ("<p><a href='update.php?id=$id'><button>Update</button></a></p>");
				echo ("<p><a href='deleteLand.php?id=$id'><button>Delete</button></a></p>");
				echo ("</div>");
				echo ("</div>");
			}	
